...
let
  ...
  packageSpecs = [
    ...
    }
    {
      name = "git-lfs";
      pkg = pkgs.git-lfs;
      description = "Git extension for large files";
    }
    {
      name = "gitui";
      pkg = pkgs.gitui;
    ...
    }
    {
      name = "navi";
      pkg = pkgs.navi;
      description = "Interactive cheatsheet tool with finder";
    }
    {
      name = "tealdeer";
      pkg = pkgs.tealdeer;
    ...
    }
    {
      name = "just";
      pkg = pkgs.just;
      description = "Command runner (like make but simpler)";
    }
    {
      name = "mcfly";
      pkg = pkgs.mcfly;
      description = "Enhanced shell history with intelligent search";
    }
    {
      name = "atuin";
      pkg = pkgs.atuin;
      description = "Shell history with AI";
    }
  ];

in
{
  options.rhodium.home.apps.terminal.utils.development = {
    enable = mkEnableOption "Rhodium's development utils";
  ...
  config = mkIf cfg.enable {
    home.packages =
      concatMap
        (spec:
          if cfg.${spec.name}.enable then [ spec.pkg ] else [ ]
        )
        packageSpecs;
  };
}
