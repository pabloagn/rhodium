...
function M.toggle_todo_done()
  ...
  if keyword then
    ...
    vim.notify(string.format("Toggled: %s → %s", keyword, keyword == "DONE" and "TODO" or "DONE"),
      vim.log.levels.INFO, { title = "TODO Toggle" })
  else
...
function M.list_buffer_todos()
  ...
  local keywords = { "TODO", "FIX", "HACK", "WARN", "PERF", "NOTE", "TEST", "DONE",
    "SEV1", "SEV2", "SEV3", "IMPR", "TESTING", "PASSED", "FAILED" }
