{lib, ...}: let
  ...
  sourceLuaFileSafe = dir: file: let
    filePath = dir + "/${file}";
  in
    if builtins.pathExists filePath
    then sourceLuaFileFromDir dir file
    else builtins.throw "Lua file not found: ${toString filePath}";

  # Source multiple Lua files and concatenate them
  sourceLuaFiles = dir: files: let
  ...
  sourceLuaFileConditional = dir: file: fallback: let
    filePath = dir + "/${file}";
  in
    if builtins.pathExists filePath
    then sourceLuaFileFromDir dir file
