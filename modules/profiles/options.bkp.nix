...
in
{
  options.mySystem.userShells = {
    ...
    defaultLoginShell = lib.mkOption {
      # What the car comes with by default
  ...
  config = lib.mkIf (!(lib.elem config.mySystem.userShells.defaultLoginShell config.mySystem.userShells.enable)) {
    # NOTE: This assertion will FAIL the build if the defaultLoginShell is not in the list of enabled shells.
    assertions = [{
